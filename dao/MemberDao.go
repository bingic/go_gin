...
func (md *MemberDao) InsertCode(sms model.SmsCode) int64 {
	fmt.Println("发送验证码 1 result=%v ", &sms)
	fmt.Println("发送验证码 2 result=%v ", &sms)
	fmt.Println("发送验证码 3 result=%v ", &sms)
	result, err := md.InsertOne(&sms)
	if err != nil {
		fmt.Println("发送验证码 nil 2 result=%v ")
		//logger.Error(err.Error())
		fmt.Println("insertCode result=%v ", result)
		fmt.Println(err.Error())
	}
	fmt.Println("发送验证码 2 result=%v ", &sms)
	return result
